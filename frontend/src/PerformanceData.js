function PerformanceData(props) {
    let currentValue = {};
    if (props.localResponse.hasOwnProperty("Time Series (5min)")) {
        //split is for truncating the data and time label that is coming back in the feed.
        let closeStr = Object.entries(props.localResponse["Time Series (5min)"])[0][1]["4. close"]
        let timeStr = Object.entries(props.localResponse["Time Series (5min)"])[0].toString().split(",", 1)
        let currentTotalStr = (closeStr * props.numberShares) - (props.origPrice * props.numberShares)
        //this.props.addToCurrentValue(currentTotalStr)
        currentValue = {
            lastClose: closeStr,
            lastTime: timeStr,
            numberShares: props.numberShares,
            currentTotalChange: currentTotalStr,
            currentChange: (closeStr) - (props.origPrice),
            currentValueId: props.currentValueId,
            ticker: props.ticker
        };
    ...
    return (
        <div
            className={'rounded-2xl p-2 border-2 shadow-xl border-gray-700 bg-white block text-gray-700 text-md font-bold'}>Performance
            Data:
